	 ...
	 <div class="container">
		...
		<div class="panel-heading">
           ...
           <?php
		    ...
		    <div class="btn-group pull-right">
				<button type='button' class="btn btn-success" data-toggle="modal" data-target="#myModal"><span class="glyphicon glyphicon-user" ></span> Nueva Mascota</button>
			</div>			
			<?php } ?>
			
			<h4><i class='glyphicon glyphicon-stats'></i> Buscar Mascota</h4>
		</div>			
			<div class="panel">
		...
		<form>
			...
			<?php
				...
				require 'modal/editar_mascota.php';
				require 'modal/registro_mascotas.php';
				require 'modal/eliminar_mascota.php';
...
$( "#editar_password" ).submit(function( event ) {
 ...
 var parametros = $(this).serialize();
  ...
  event.preventDefault();
      ...
      function obtener_datos(Id, Id_Cliente, Nombre_Mascota, Especie, Raza, Sexo, nacimiento, Descripcion){
		
				
			$("#id_mascota").val(Id);
	        $("#id_cliente").val(Id_Cliente);
			$("#nom_mascota").val(Nombre_Mascota);
	 		$("#especie").val(Especie);
	  		$("#raza").val(Raza);
			$("#sexo").val(Sexo);
	        $("#nazi").val(nacimiento);
...
$('#bd-desde').on('change', function(){
		...
		var url = 'ajax/busca_mascotas_fecha.php';
		$.ajax({
		type:'POST',
		url:url,
...
function reportePDF(){
	var desde = $('#bd-desde').val();
	var hasta = $('#bd-hasta').val();
	window.open('reporte/remascotas.php?desde='+desde+'&hasta='+hasta);
}
