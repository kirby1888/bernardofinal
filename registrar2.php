...
if (!$_POST["pass1"] or !$_POST["txt_nc"] or !$_POST["correo"] or !$_POST["txt_us"] or !$_POST["combo_estado"] or !$_POST["rol"]) {
	...
	$lo=("select correo_electronico from tbl_usuario where correo_electronico='$correo'");
		...
		$la=("select usuario from tbl_usuario where usuario='$us'");
 			...
 			if ($fb) {
				...
				if (!$fss) {
					$consulta=("insert into tbl_usuario (nombre_usuario,usuario,contrasena,correo_electronico,estado_usuario,id_rol, fecha_creacion,activacion,fecha_cambio_contrasena) values('$nom', '$us','$pass_hash','$correo','$estado',$rol,'$f1',1,NOW())");
